...
def prepare_for_FFT(ds,n, label):
    ...
    F       = ds[label].to_numpy()         # this is likely already pre-normalized, but we'll call it F for clarity
...
def ds_dp(hkl_ind, N123, n):
    ...
    ds_da =  np.cos(2*np.pi*(h_sc @ m)) # the prefactor np.prod(N123) should cancel out in rescaling in the fcn
...
def ds_sc_dp(s,lam, hkl_index, mnp_index, N123, n):
    ...
    """
    ds_sc_dp_grad = ds_sc_ds(s,lam).reshape(-1,1).repeat(2*n**3,axis=1) * ds_dp(hkl_index, N123, n)
#     np.set_printoptions(precision=3)
    return ds_sc_dp_grad

def dE_dp(F, s, lam, hkl_index, N123, n):
    # E = F*s_sc (not inverse)
    return (F*ds_sc_ds(s,lam)).reshape(-1,1).repeat(2*n**3,axis=1) * ds_dp(hkl_index, N123, n)

def dPE_dp_centric(E_sc, E, E0, nH, n_p, dEdp_res): #np=2*n**3 or p.shape[0]
    """     dP(E)/dp = dP(E)/dE * dE/dp, with p the input parameters.
    """
    return (wilson.dPE_dE_centric( E_sc)*dEsc_dE(E,E0,nH)).reshape(-1,1).repeat(n_p,axis=1)*dEdp_res
        
def dPE_dp_acentric(E_sc, E, E0, nH, n_p, dEdp_res):
    """     dP(E)/dp = dP(E)/dE * dE/dp, with p the input parameters.
    """
    return (wilson.dPE_dE_acentric(E_sc)*dEsc_dE(E,E0,nH)).reshape(-1,1).repeat(n_p,axis=1)*dEdp_res
    
def dLn_dp(params, E_sc, E, E0, s, s_sc, lam, nH, n):
    PE_c  = wilson.wilson_dist_normalized(E_sc, centric=True,  nargout=1).reshape(-1,1).repeat(2*n**3,axis=1) +1e-6
    PE_ac = wilson.wilson_dist_normalized(E_sc, centric=False, nargout=1).reshape(-1,1).repeat(2*n**3,axis=1) +1e-6
    
#     dE_dp(F, s, lam, hkl_index, N123, n)
    dEdp_res = dE_dp(params[ "F"], s, lam, params["hkl_ind"], params["L"], n)    
    return (dPE_dp_centric( E_sc,E,E0,nH,2*n**3, dEdp_res)/PE_c )*   params["centric" ].reshape(-1,1).repeat(2*n**3,axis=1)+\
           (dPE_dp_acentric(E_sc,E,E0,nH,2*n**3, dEdp_res)/PE_ac)*(1-params["centric"]).reshape(-1,1).repeat(2*n**3,axis=1)+\
           (ds_sc_dp(s,lam, params["hkl_ind"], params["mnp_ind"], params["L"], n)\
            /s_sc.reshape(-1,1).repeat(2*n**3,axis=1))

def anisotropic_scaling_to_1_FFT_wilson_loss_fast(p_sc, params, bUse = True, correct = True, grad = True, weights=True, nargout=1):
    """
    ...
    total_scale = (total_scale_grid[hkl_ind[:,0],hkl_ind[:,1],hkl_ind[:,2]]).real  # it might be more appropriate to add the negative freq component as a high freq component with neg phase etc.
    ...
    EP_corr_sc  = (EP_corr**nH/(1+(EP_corr/E0)**nH))**(1/nH) # we need this nonlinearity to stop running out of the range 
    ...
    loss = np.sum(loss_E_ac[(params["centric"]==False) & bUse]) + np.sum(loss_E_c[params["centric"] & bUse]) 
    
    if grad:
        per_obs_grad = dLn_dp(params, E_sc=EP_corr_sc, E=EP_corr, E0=E0, s=total_scale, s_sc=total_scale_sc, lam=lam, nH=nH, n=n)
        total_loss_grad = -2*np.sum(np.repeat(w[bUse].reshape(-1,1),per_obs_grad.shape[1],1)*per_obs_grad[bUse,:],axis=0)
        if not (total_loss_grad.shape[0] == 2*n**3):
            print("Warning: the output size of the gradients is incorrect. bUse should be an array, not a scalar boolean")
