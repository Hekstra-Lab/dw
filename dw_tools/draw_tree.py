...
def prepare_tree_pgm(list_of_nodes, list_of_edges, list_of_node_labels, list_of_edge_labels, root=0):
    ...
    for n in list_of_nodes:
        dh_add_node(_pgm, list_of_nodes[n], list_of_node_labels[n], \
                    _coordinates[n,0], _coordinates[n,1])
    for e in list_of_edges:    
        _pgm.add_edge(list_of_nodes[e[0]], list_of_nodes[e[1]], 
                     label=f"{list_of_edge_labels[list_of_edges.index(e)]}",label_params={'fontsize':14})
    return _pgm
