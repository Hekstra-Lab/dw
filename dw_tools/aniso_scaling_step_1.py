...
def ds_sc_dp_aniso(s,lam, p, rlp, eps):
    ...
    """
    ds_sc_dp_grad = ds_sc_ds(s,lam).reshape(-1,1).repeat(p.shape[0],axis=1) * ds_dp_aniso(p, rlp, eps)
#     np.set_printoptions(precision=3)
    return ds_sc_dp_grad

def dE_dp_aniso(F, s, lam, p, rlp, eps):
    # E = F*s_sc (not inverse)
    return (F*ds_sc_ds(s,lam)).reshape(-1,1).repeat(p.shape[0],axis=1) * ds_dp_aniso(p, rlp, eps)
...
def dLn_dp_aniso(params, E_sc, E, E0, s, s_sc, lam, nH, n_p,p): #n_p as above
    PE_c  = wilson.wilson_dist_normalized(E_sc, centric=True,  nargout=1).reshape(-1,1).repeat(n_p,axis=1) 
    PE_ac = wilson.wilson_dist_normalized(E_sc, centric=False, nargout=1).reshape(-1,1).repeat(n_p,axis=1) 
    
#     dE_dp(F, s, lam, hkl_index, N123, n)
    dEdp_res = dE_dp_aniso(params[ "F"], s, lam, p, params["rlp"], params["eps"])
    return (dPE_dp_centric( E_sc,E,E0,nH,n_p,dEdp_res)/PE_c )*   params["centric" ].reshape(-1,1).repeat(n_p,axis=1)+\
           (dPE_dp_acentric(E_sc,E,E0,nH,n_p,dEdp_res)/PE_ac)*(1-params["centric"]).reshape(-1,1).repeat(n_p,axis=1)+\
           (ds_sc_dp_aniso(s,lam, p, params["rlp"], params["eps"])/s_sc.reshape(-1,1).repeat(n_p,axis=1))

def anisotropic_scaling_to_1_wilson_loss(p, ds, label="FP", suffix="", nH=4, E0=20, lam=10, grad=True, weights=False, nargout=1):
    """
    ...
    EP_corr_sc = (EP_corr**nH/(1+(EP_corr/E0)**nH))**(1/nH) # we need this nonlinearity to stop running
                                        # out of the range 
...
#    print(np.amax(total_scale_sc)) 
    ...
    grad_per_obs = dLn_dp_aniso(params, E_sc=EP_corr_sc, E=EP_corr, E0=E0, s=total_scale, s_sc=total_scale_sc, lam=lam, nH=nH, n_p=len(p),p=p)
