...
class RiceWoolfson_from_parent:
    ...
    def sample(self, sample_shape=(), seed=None, name='sample', **kwargs):
        return np.where(
            self._centric,
            self._woolfson.rvs(size=sample_shape, random_state=seed) + self.eps,
...
class RiceWoolfson_for_DAG:
    ...
    def log_prob(self, x):
        ...
        if self._root_R is None:
            ...
            self._log_prob_array[:, self._root] = RiceWoolfson_from_parent(
                parent_R=1, rDW=0, Sigma=self._Sigma, centric=self._centric
            ).log_prob(x[:, self._root])
        # now we follow the tree
        for e in range(self._n_edges):
            self._log_prob_array[:, self._list_of_edges[e][1]] = RiceWoolfson_from_parent(
                parent_R=x[:, self._list_of_edges[e][0]],
    ...
    def prob(self, x):
        ...
        if self._root_R is None:
            # we sample from the Wilson distribution
            self._prob_array[:, self._root] = RiceWoolfson_from_parent(
                parent_R=1, rDW=0, Sigma=self._Sigma, centric=self._centric
            ).prob(x[:, self._root])
        # now we follow the tree
        for e in range(self._n_edges):
            self._prob_array[:, self._list_of_edges[e][1]] = RiceWoolfson_from_parent(
                parent_R=x[:, self._list_of_edges[e][0]],
                rDW=self._list_of_rDW[e],
                Sigma=self._Sigma,
                centric=self._centric,
    ...
    def sample(self, n_samples, seed=None, name='sample', **kwargs):
        ...
        if self._root_R is None:
            ...
            self._samples_array[self._root, 0, :] = RiceWoolfson_from_parent(
                parent_R=1, rDW=0, Sigma=self._Sigma, centric=self._centric
            ).sample(sample_shape=(n_samples,), seed=None, name='sample', **kwargs)
        else:
            self._samples_array[self._root, :, :] = np.broadcast_to(
                self._root_R.reshape(self._n_refl, -1), (self._n_refl, n_samples)
            )
        # now we follow the tree
        for e in range(self._n_edges):
            for f in range(self._n_refl):  # Just to keep things simpler for now
                self._samples_array[self._list_of_edges[e][1], f, :] = RiceWoolfson_from_parent(
                    parent_R=self._samples_array[self._list_of_edges[e][0], f, :].flatten(),
                    rDW=self._list_of_rDW[e],
                    Sigma=self._Sigma,
                    centric=self._centric,
                ).sample(sample_shape=(n_samples,), seed=None, name='sample', **kwargs)
