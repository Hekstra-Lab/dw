...
def fit_ab(ds,labels=["EP_1","EP_2"],dHKL_label="dHKL_1", dHKL_bin_label="dHKL_bin"):
    ...
    result = least_squares(fit_ab_ds_residual, [0.5,1], args=(ds,labels,dHKL_label,dHKL_bin_label),\
...
def eff_r_dw_per_hkl(ds,a,b,label, dHKL_label,inplace=True):
    ...
    rho_obs_ac = 1/np.sqrt(rho_DW**-2 + var_eta.astype(float)/varW_acentric) # not sure why I had to set the data type explicitly; otherwise just "object"
